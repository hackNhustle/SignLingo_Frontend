  ...
  export default function Translator() {
    ...
    const getSpeechErrorMessage = (errorCode?: string) => {
      switch (errorCode) {
        ...
        case 'service-not-allowed':
          return 'Microphone access was blocked. Please allow microphone permission in Chrome and try again.';
        case 'audio-capture':
        ...
        case 'network':
          return 'Speech recognition hit a network issue. Please check your connection and try again.';
        case 'aborted':
          return '';
        default:
    ...
    const getMicrophonePermissionError = (err: any) => {
      ...
      switch (errorName) {
        ...
        default:
          return 'Unable to access the microphone. Please check browser permissions and try again.';
      }
    };
    ...
    useEffect(() => {
      ...
      if (!recognition || typeof recognition.addEventListener !== 'function') {
        return;
      }

      const handleRecognitionError = (event: any) => {
        const nextError = getSpeechErrorMessage(event?.error);
    ...
    useEffect(() => {
      if (!isMicrophoneAvailable) {
        setError('Microphone access is unavailable. Please allow microphone permission in your browser settings.');
      }
    }, [isMicrophoneAvailable]);
    ...
    const handleMicClick = async () => {
      if (listening) {
        ...
        if (!window.isSecureContext) {
          setError('Speech recognition requires a secure context in production. Please use HTTPS.');
          return;
        }

        if (!browserSupportsSpeechRecognition || !nativeSpeechRecognitionSupported) {
        ...
        try {
          ...
          setError(getMicrophonePermissionError(err));
        }
      }
    };
