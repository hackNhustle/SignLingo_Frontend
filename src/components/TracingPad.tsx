  ...
  return (
    <div className="relative w-full max-w-md h-screen flex flex-col bg-background-light dark:bg-background-dark mx-auto shadow-2xl overflow-hidden">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 w-full relative">
        <div 
          ref={containerRef}
          className="bg-white dark:bg-[#1a2c2b] w-full aspect-[4/5] max-h-[60vh] rounded-3xl shadow-soft relative flex items-center justify-center overflow-hidden border border-slate-100 dark:border-slate-800"
        >
          {/* Background Grid */}
          ...
          {/* Small Letter Reference (top corner) */}
          <div className="absolute top-4 right-4 pointer-events-none z-0 bg-white/50 dark:bg-black/30 rounded-lg px-3 py-2">
            <span className="text-5xl font-bold text-slate-400 dark:text-slate-500 leading-none select-none">
              {targetLetter}
            </span>
          </div>
                ...
                if (debug && canvasRef.current) {
                  ...
                  if (data.lines && data.lines.length > 0) {
                    ...
                    if (lastLine.points && lastLine.points.length > 0) {
                      ...
                      const nearestCheckpoint = LETTER_CHECKPOINTS[targetLetter]?.[0] || { x: 0, y: 0 };
          ...
          {debug && LETTER_CHECKPOINTS[targetLetter]?.map((checkpoint, index) => (
            <div
              ...
              style={{
                left: `${checkpoint.x * canvasScale.scaleX}px`,
                top: `${checkpoint.y * canvasScale.scaleY}px`,
